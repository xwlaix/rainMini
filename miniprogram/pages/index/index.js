...
var n = {
  ...
  getAwards: function (a) {
    var e = this;
    console.log("结果 " + a.detail.s_awards),
      this.setData({
        s_awards: a.detail.end ? "🐾  ▒▒▒▒  🐾" : a.detail.s_awards,
        share: !!a.detail.end,
      }),
      console.log("结果 " + this.data.s_awards),
      console.log("旋转结果 " + a.detail + "  " + a.detail.end),
      a.detail.end ||
        setTimeout(function () {
          e.result.show();
        }, 100);
  ...
  xiaojueding: function (a) {
    ...
    if (!e.data.zhuanflg) return e.zhuanpan.switchZhuanpan(t);
  },
  onShow: function (t) {
    console.log("============onShow============");
    var n = this,
      s = wx.getStorageSync("switchTab"),
      i = wx.getStorageSync("all"),
      o = wx.getStorageSync("all");
    this.onGethomeDecision();
    (
      (i = e.globalData.defaultJueding ? o : e.globalData.myJueding ? i : o)
    ),
      n.setData({
      ...
      a.isNull(s) ||
        ...
        setTimeout(function () {
          for (var a in i)
            if (i[a].id == s) {
              n.zhuanpan.switchZhuanpan(i[a], !0),
                n.setData({
                  zhuanflg: !1,
                });
              break;
  ...
  generate: function () {
    ...
    for (var e = [], t = 0; t < a.data.awardsConfig.awards.length; t++)
      e.push(a.data.awardsConfig.awards[t].name);
    a.makeImageCanvas(
      "shareCanvas",
      a.data.awardsConfig.option,
      e,
      a.data.colorArr,
      a.data.fontArr,
      a.data.sizeArr,
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: a.isNull(e.globalData.shareTitle)
        ? "一起来玩'" + e.globalData.title + "'吧"
        : e.globalData.shareTitle,
  ...
  makeImageCanvas: function (a, e, t, n, s, i, o, l, r, d, c, h, g, u, f) {
    ...
    for (var b = [], A = 0; A < w.length; A++) {
      var C = parseInt(A / l) + 1,
        D = A % l,
        T = n[A],
        S = v.arrayRandomTakeOne(s),
        y = v.arrayRandomTakeOne(i);
      b.push(y);
      var F = 0,
        z = C * d;
      if (0 == D) F = 0;
      ...
      (x.fillStyle = T),
        (x.font = S + " small-caps normal " + y + "px Arial"),
        x.fillText(w[A], F, z);
    }
    x.setFillStyle("white"), x.fillRect((c - g) / 2, (h - u) / 2, g, u);
    for (var I = [], k = 0; k < e.length; k++) I.push(e[k]);
    var R = u - 10 - g,
      M = parseInt(R / e.length);
    M = M > 50 ? 50 : M;
    for (var P = 0; P < I.length; P++)
      (x.fillStyle = "#000000"),
        (x.font = "normal small-caps normal " + M + "px Arial"),
        x.setTextAlign("center"),
        x.fillText(I[P], c / 2, (h - u) / 2 + 5 + M * (P + 1));
    x.drawImage(f, (c - g) / 2 + 5, h - (g + (h - u) / 2), g - 10, g - 10),
      wx.drawCanvas({
        canvasId: a,
  ...
  onGethomeDecision: async function () {
    ...
    const lastResult = await wx.cloud.callFunction({
      name: "home",
      data: {
        action: "homeDecision",
        data: {
    ...
    if (wx.getStorageSync("switchTab")) {
      this.setData({
        xiaojuedingShowArr: result.result.data,
      });
    } else {
      this.setData({
        xiaojuedingShowArr: result.result.data,
        xiaojuedingArr: lastResult.result.data,
      });
    }
  },
};
